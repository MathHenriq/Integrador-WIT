...
--   Atualização 15: resumo curto não derruba mais a publicação
--
--   Cole no SQL Editor. Pode rodar quantas vezes quiser.
--
-- =====================================================================
--
-- O catálogo exige resumo com dez letras (`aulas_resumo_check`). Uma aula
-- publicada sem descrição — ou com uma descrição de uma palavra só — caía
-- nessa trava, e o painel mostrava o texto cru do Postgres: "new row for
-- relation aulas violates check constraint". A aula não era registrada.
--
-- Duas correções, uma para a causa e outra para o resto:
--
--   1. sem descrição, o resumo passa a ser montado do tema com a escola,
--      que sempre tem tamanho;
--   2. abrir a atividade no catálogo virou um passo que pode falhar sem
--      levar o registro da aula junto — a aula fica salva e a tela conta
--      o que aconteceu.
...
begin
  ...
  -- A aula realizada é o que não pode se perder. Se o catálogo recusar a
  -- atividade por algum motivo, a aula fica registrada do mesmo jeito e a
  -- tela conta o que houve, em vez de a tela inteira virar um erro cru de
  -- banco de dados.
  if p_virar_atividade then
   begin
    ...
    -- O catálogo pede um resumo com pelo menos dez letras. Documento sem
    -- descrição, ou com uma descrição de uma palavra só, não pode derrubar
    -- o registro da aula por causa disso.
    if length(btrim(coalesce(v_resumo, ''))) < 10 then
      v_resumo := v_titulo || ' — aula do projeto integrador na ' || v_escola.nome || '.';
    end if;
   ...
   exception when others then
     v_aula_id := null;
     v_nova_aula := false;
     v_aviso := 'A aula foi registrada, mas não consegui abrir a atividade no catálogo: ' || sqlerrm;
   end;
  end if;
...
select 'Aula sem descrição também entra no catálogo.' as resultado;
